...
public class User {
    ...
    public void loadPerms(String s) {
        ...
        if (Bukkit.getServer().getPlayer(UUID.fromString(getUuid())) != null) {
            attachment = Bukkit.getServer().getPlayer(UUID.fromString(getUuid())).addAttachment(Kingdoms.getCore());
            if (!perms.isEmpty()) {
                for (String str : perms) {
                    attachment.setPermission(str, true);
                }
            }
            if (Kingdoms.getCore().getIdToRank().containsKey(getRank())) {
                for (String perm : Kingdoms.getCore().getIdToRank().get(getRank()).getPermissions()) {
                ...
                if (Kingdoms.getCore().getIdToRank().get(getRank()).getInherits() != null) {
                    for (String inheritRank : Kingdoms.getCore().getIdToRank().get(getRank()).getInherits()) {
                        for (String inhereRankPerm : Kingdoms.getCore().getIdToRank().get(inheritRank).getPermissions()) {
                            attachment.setPermission(inhereRankPerm, true);
                        }
                        if (Kingdoms.getCore().getIdToRank().get(inheritRank).getInherits() != null) {
                            for (String tmp : Kingdoms.getCore().getIdToRank().get(inheritRank).getPermissions()) {
                                attachment.setPermission(tmp, true);
                            }
                            if (Kingdoms.getCore().getIdToRank().get(inheritRank).getInherits() != null) {
                                for (String inhereRank : Kingdoms.getCore().getIdToRank().get(inheritRank).getInherits()) {
                                    if (Kingdoms.getCore().getIdToRank().get(inhereRank).getPermissions() != null) {
                                        for (String inhereRankPerm : Kingdoms.getCore().getIdToRank().get(inhereRank).getPermissions()) {
